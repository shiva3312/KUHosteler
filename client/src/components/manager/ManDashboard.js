...
const AdminDashboard = ({ history }) => {
  ...
  const loadUsers = () => {
    ...
    user.mealInfoList.forEach((rec) => {
      ...
      months.push(
        rec.auditedDate.slice(4, 7) + " " + rec.auditedDate.slice(-4)
      );
  ...
  const basicInfoCards = () => {
    ...
    return (
      <div className="text-box fadeUp animate">
        {hour >= 12 ? (
          hour >= 16 ? (
            <h2 className="welcome p-1 pt-3">
              Good Evening, {user.fname} {user.lname}
            </h2>
          ) : (
            <h2 className="welcome p-1 pt-3">
              Good Afternoon, {user.fname} {user.lname}
            </h2>
          )
        ) : (
          <h2 className="welcome p-1 pt-3">
            Good Morning, {user.fname} {user.lname}
          </h2>
        )}
              ...
              <tbody>
                {allListedGuest.map((guest, i) => (
                  <tr key={i}>
                    ...
                    {guest.guestType == 0 ? (
                      <td className="th1">Normal</td>
                    ) : (
                      <td className="th1">Official</td>
                    )}
          ...
          <div className="shadow tbl-header">
            <table cellPadding="0" cellSpacing="0" border="0">
              <thead>
                <tr>
                  {" "}
                  <th className="th1 sl col-1">SL</th>
