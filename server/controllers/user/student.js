    ...
    if (err || !user) {
      ...
      user.save((err) => {
        if (err)
          ...
          return res.status(200).json({ info: "sucessfully changed" });
        }
      });
    }
  });
...
exports.helpdesk = (req, res) => {
  User.findOne(
    { hostelName: req.profile.hostelName, profileType: 1 },
    (err, manager) => {
      if (err || !manager) {
        return res.status(400).json({
...
exports.addguest = (req, res) => {
  ...
  let validationUnder30day =
    startDate <= today30Ahead && endDate <= today30Ahead;
  ...
  if (!(validationUnder30day && startTOendDate && afterToday)) {
    ...
    User.findOne(
      { profileType: 1, hostelName: req.profile.hostelName },
          ...
          User.findById(req.profile._id, function (err, student) {
            if (err) {
              ...
              while (i <= endDate) {
                ...
                if (
                  i.getDate() == startDate.getDate() &&
                  i.getMonth() == startDate.getMonth() &&
                  i.getFullYear() == startDate.getFullYear()
                ) {
                  newGuest = {
                    date: pushDate,
                  ...
                  i.getDate() == endDate.getDate() &&
                  i.getMonth() == endDate.getMonth() &&
                  i.getFullYear() == endDate.getFullYear()
                ) {
                  newGuest = {
                    date: pushDate,
                    name: name,
              ...
              if (req.profile.membership == 2)
                ...
                return res.json({
                  error: "You are no longer member of This Hostel",
                });
              else
...
exports.updateguest = (req, res) => {
  ...
  User.findOneAndUpdate(
    {
      _id: req.profile._id,
      active_guest_list: { $elemMatch: { _id: guestId } },
    },
    {
      $set: {
