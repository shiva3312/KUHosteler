  ...
  await boundTime.findOne({ hostelName: hostelName }, (err, manager) => {
    ...
    if (err) console.log(err);
      ...
      User.find({ hostelName: hostelName }, (err, students) => {
          ...
          students.forEach((student) => {
            ...
            student.active_guest_list.forEach((guest) => {
              ...
              if (!(isMorning && guest.mealTime == "night")) {
                if (
                  currDateInString.slice(0, 15) === guest.date.slice(0, 15) &&
                  guest.mealStatus == 1
                ) {
                  let newGuestRec = {
                    _id: new mongoose.Types.ObjectId(),
                  ...
                  if (student.profileType == 1) countOfficialGuestMeal++;
        ...
        User.findOne(
          { hostelName: hostelName, profileType: 1 },
          (err, manager) => {
            if (err) console.log(err);
            else {
                  ...
                  if (isMorning) {
                    ...
                    User.findOneAndUpdate(
                      ...
                      function (err, guestFound) {
                        if (err) {
                          console.log(err);
                        } else console.log("saved in data");
                      }
                    );
                  }
                  // if(night)
          ...
          student.active_guest_list.forEach(function (guest) {
            ...
            if (guest.mealTime == "morning" && guestDate == push_date) {
              ...
              User.findOneAndUpdate(
                {
                  _id: student._id,
                  active_guest_list: { $elemMatch: { _id: guest._id } },
                },
                {
                  $set: {
          ...
          student.activity.forEach((record, i) => {
            ...
            if (record.date == push_date) {
              ...
              student.active_guest_list.forEach((guest, i) => {
                ...
                if (dateValidation && mealshiftValidation) {
                  var guestName = guest.name;
                  allPushedGuest_In_ActiviyRecord.forEach(function (
                    pushedGuest,
                    i
                  ) {
